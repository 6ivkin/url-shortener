...
func (s *Storage) SaveURL(urlToSave string, alias string) (int64, error) {
	...
	if err != nil {
		if sqliteErr, ok := err.(sqlite3.Error); ok && sqliteErr.ExtendedCode == sqlite3.ErrIoErrAccess {
...
func (s *Storage) GetURL(alies string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("%s: prepere statement: %w", op, err)
	}

	var resURL string
	err = stmt.QueryRow(alies).Scan(&resURL)
	if err != nil {
